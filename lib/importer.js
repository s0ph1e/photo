	...
	function getDestination (source) {
		return exif.read(source.path, 16)
			.then(exif => {
				...
				var month = lpadz(date.getMonth() + 1);
				var day = lpadz(date.getDate());
				var hours = lpadz(date.getHours());
				var minutes = lpadz(date.getMinutes());
				var seconds = lpadz(date.getSeconds());
				return path.join(
					output,
					util.format('%s-%s-%s-%s-%s-%s-%s.jpg', year, month, day, hours, minutes, seconds, camera)
				);
			})
